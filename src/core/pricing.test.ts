...
test("netProfit is positive when gross edge beats both taker fees", () => {
  // Buy 100000, sell 100600, 0.1% each side, 0.1 BTC.
  const net = netProfit(100000, 100600, 0.1, 0.001, 0.001);
  // proceeds = 100600*0.1 - 100600*0.1*0.001 = 10060 - 10.06 = 10049.94
  // cost     = 100000*0.1 + 100000*0.1*0.001 = 10000 + 10   = 10010
  assert.ok(Math.abs(net - 39.94) < 1e-9, `got ${net}`);
});

test("netProfit is negative when fees exceed gross edge", () => {
  // Matches the legacy inline assertion: 0.1% gross, 0.1% fees both sides.
