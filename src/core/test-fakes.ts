/**
 * Lightweight in-memory port fakes for characterization tests. They implement
 * the same ports the production adapters do, but with no config/runtime/IO, so
 * tests fully control fees, thresholds, time and ids.
 */
import {
  EXCHANGE_IDS,
  type CircuitState,
...
/** Mutable policy with production defaults; override any field per test. */
...
/** Captures everything the engine/rebalancer push, for assertions. */
...
/** Risk gate with a fixed verdict; records calls. */
...
/** Records execute() calls; returns a deterministic stub trade. */
...
/** In-memory inventory; records transfers. */
...
/** In-memory quote book; freshness mirrors OrderBookManager (now - recvTs <= staleMs). */
...
/** Build an order book with explicit top levels (depth padded if requested). */
